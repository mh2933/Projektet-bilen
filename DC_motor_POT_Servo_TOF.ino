...
/*void tof_sensor(void)
{
  uint16_t sensor_val = pulseIn(tof_sensor, HIGH);
  Serial.print(sensor_val);
 if (sensor_val < 1200)
{
  myservo.write(180);
}   
 else if (sensor_val > 1200 && sensor_val < 1900)
{
  myservo.write(0);
} 
 else
...
//servo();
//tof_sensor();
uint16_t sensor_val = pulseIn(tof_sensor_read, HIGH);
